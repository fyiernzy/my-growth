...
( git -c core.pager=cat log $COLOR_FLAG --graph --decorate --oneline "${BASE_COMMIT}^..${HEAD_SHA}" || true ) \
| awk '
  BEGIN { c=0; m=0; omitted=0; }
  {
    if ($0 ~ / Merge( branch| pull request)? / || $0 ~ /^.*Merge /) m++
    c++
    if (m > 3 || c > 10) {
      if (!omitted) {
        omitted=1
...
if [ "$merge_commits" -gt 3 ] || [ "$total_commits" -gt 10 ]; then
  printf " \n"
  echo -e "... (Omitted for brevity, run ${CYAN_BOLD} git log ${COLOR_FLAG} --graph --decorate --oneline ${BASE_COMMIT}^..${HEAD_SHA} ${RESET} locally for full results.)"
  printf " \n"
fi
