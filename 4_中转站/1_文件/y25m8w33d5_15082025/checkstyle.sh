...
cecho "$YELLOW_BOLD" "======== Commit History =========="
COLOR_FLAG="--color=always"

# Count total commits in range (inclusive of both ends)
total_commits=$(git rev-list --count "${BASE_COMMIT}^..${HEAD_SHA}")

# Count merge commits in range
merge_commits=$(git rev-list --merges --count "${BASE_COMMIT}^..${HEAD_SHA}")

# Show commit history with both endpoints
git log $COLOR_FLAG --graph --decorate --oneline \
  "${BASE_COMMIT}^..${HEAD_SHA}" \
  | awk '
    BEGIN { commit_count=0; merge_count=0; }
    {
      if ($0 ~ /^.*Merge branch/ || $0 ~ /^.*Merge pull request/ || $0 ~ /^.*Merge /) {
        merge_count++
      }
      commit_count++
      if (merge_count > 3 || commit_count > 10) {
        print "...(Omitted for brevity, run the above git log command locally for full results.)"
        exit
      }
      print
    }
...
# Add omission note if thresholds exceeded
if [ "$merge_commits" -gt 3 ] || [ "$total_commits" -gt 10 ]; then
  echo
  GIT_COMMAND="git log ${COLOR_FLAG} --graph --decorate --oneline ${BASE_COMMIT}^..${HEAD_SHA}"
  echo "...(Omitted for brevity, run'${GIT_COMMAND} locally for full results.)"
fi

CHANGED="$(git diff --name-only --diff-filter=ACMRTUXB "$BASE_COMMIT" "$HEAD_SHA" -- ':(top,glob)**/*.java')"

if [[ -z "$CHANGED" ]]; then
  printf "\nNo changed Java files detected. Skipping Checkstyle.\n"
  mkdir -p build/reports/checkstyle
  echo '[]' > gl-code-quality-report.json
...
if $vcl_ok; then
  ...
  tmp_out="$(mktemp)"
  if "${gradle_cmd}" --no-daemon --console=plain --warning-mode=fail -Dorg.gradle.welcome=never \
