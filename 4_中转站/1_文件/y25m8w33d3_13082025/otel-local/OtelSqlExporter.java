...
public class OtelSqlExporter implements SpanExporter {
    ...
    // Attribute keys
    private static final AttributeKey<String> DB_STATEMENT_KEY = AttributeKey.stringKey(
        DB_STATEMENT);
    ...
    private static final AttributeKey<String> DB_OPERATION_KEY = AttributeKey.stringKey(
        DB_OPERATION);
    private static final AttributeKey<String> DB_SQL_TABLE_KEY = AttributeKey.stringKey(
        DB_SQL_TABLE);

    // Others
    private static final Logger logger = Logger.getLogger(OtelSqlExporter.class.getName());
    private static final String FORMAT = " %-18s: %s%n";
    private static final org.slf4j.Logger log = LoggerFactory.getLogger(OtelSqlExporter.class);

    private final AtomicBoolean shutdown = new AtomicBoolean();
    private final BasicFormatterImpl formatter = new BasicFormatterImpl();

    @Override
    public CompletableResultCode export(Collection<SpanData> spanDatas) {
        if (shutdown.get()) {
        ...
        spanDatas.stream()
            ...
            .forEach(spanData -> {
                ...
                ObservabilityUtils.withMdc(mdc, () -> logger.info(message));
            });
...
//               + String.format(FORMAT, SQL, sqlInfo.sql);
    ...
    private SqlInfo toSqlInfo(SpanData spanData) {
        long executionMs = TimeUnit.NANOSECONDS.toMillis(
            spanData.getEndEpochNanos() - spanData.getStartEpochNanos());

        Attributes attributes = spanData.getAttributes();

        String dbSystem = orElseGet(attributes.get(DB_SYSTEM_KEY), () -> "");
        String dbName = orElseGet(attributes.get(DB_NAME_KEY), () -> "");
        String dbOperation = orElseGet(attributes.get(DB_OPERATION_KEY), () -> "");
        String dbTable = orElseGet(attributes.get(DB_SQL_TABLE_KEY), () -> "");
        String sql = Optional.ofNullable(attributes.get(DB_STATEMENT_KEY)).map(formatter::format)
            .orElse("");
    ...
    private <T> T orElseGet(T value, Supplier<T> defaultValue) {
